...
/*   Updated: 2024/05/28 17:26:35 by najeuneh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../inc/libft.h"

static int	ft_check(char *tri, char str)
{
	int	i;
...
static int	ft_count(char *str, char *tri)
{
	...
	if (!str || !tri)
		return (0);
	j = ft_strlen(str) - 1;
	while (ft_check(tri, str[i]) == 1)
		i++;
	k = j;
	while (ft_check(tri, str[j]) == 1 && j >= 0)
	{
		x++;
		j--;
	}
...
char	*ft_strtrim(char const *s1, char const *set)
{
	...
	if (!fnsh)
		return (NULL);
	while (ft_check((char *)set, str[i]) == 1)
		i++;
	while (str[i] && i <= j)
	{
		fnsh[x++] = str[i++];
